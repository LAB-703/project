...
###########################데이터 처리 공간#################################################################
sports=pd.read_excel("공유누리체육시설(서울).xlsx")
...
seoul_gu = gpd.read_file(seoul_gu, encoding='euc-kr')
...
if select_event == '0️⃣ 서울시 공공체육시설 지도':
    HtmlFile = open("공공체육시설(서울) 클러스터링 시각화(최종).html", 'r',encoding='utf-8')
...
if select_event == '4️⃣ 수영장':
    ...
    with rate :
        ...
        menu_list=st.text_area("*작성한 후기는 공개되며, 부적절한 후기는 삭제될 수 있습니다.",
                               max_chars=140,placeholder="서로를 배려하는 마음을 담아 작성해 주세요.")
